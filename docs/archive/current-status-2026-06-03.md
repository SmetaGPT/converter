...
- state layer получил lightweight entry point `docs/state-snapshot.md`, а cold-start для локальных задач теперь идёт через fast path без обязательного чтения полного state layer;
- добавлен локальный automation helper `scripts/classify_agent_scope.py` и workspace task `Agent: classify scope`, чтобы агентный старт чаще оставался на fast path и не открывал дорогой state layer без реальной необходимости;
- добавлен `scripts/build_state_snapshot.py`, который пересобирает `docs/state-snapshot.md` из `current-status/current-sprint/release-status/agent-feature-spine`, а `scripts/validate_harness_assets.py` теперь ловит drift этого entry point;
- добавлен `scripts/estimate_context_tokens.py`, который даёт rough token budget по startup docs, явным файлам и inline text, а также умеет excerpt-mode по первым N строкам для более реалистичного read budget; это локальная эвристика prompt-budget, а не точный backend usage meter;
- добавлен `scripts/agent_preflight.py`, который в одном запуске объединяет scope classifier, recommended first reads, first validation target, rough startup budget по excerpt-mode и условный handoff summary для resume/state-aware задач;
- добавлен `scripts/build_agent_handoff.py`, который собирает compact handoff payload из `docs/state-snapshot.md`, последней telemetry entry и текущего `git status`, чтобы следующий чат стартовал с явного anchor, residual risk и next step;
...
44. GUI доведён до operator-grade v0.2 surface: progress, current file, cancel, open output, summary counts и явный selector formula OCR-модели; по умолчанию он ведёт в локальный `PaddleOCR`, а альтернативный cloud path переключает OCR stage на `Mathpix` без изменения downstream postprocess contour.
...
88. Добавлен opt-in local formula OCR backend: `FORMULA_RECOGNITION_LOCAL_BACKEND=tesseract|paddleocr` теперь включает локальный raster OCR fallback между WMF hints и provider stage, `run.json` безопасно сериализует выбранный `local_backend`, а focused config/formula-recognition tests подтверждают generic local-backend path и provider fallback без утечки `api_key`; реальный `paddleocr` runtime при этом остаётся opt-in и требует отдельной установки `paddleocr` + `paddlepaddle`. Июньский follow-up по extraction surface закрыл исходный blocker: `docx-native-route` теперь сохраняет `asset_ref` на residual `formula` units с drawing-backed media, поэтому rerun по `421/пр` дал реальный OCR-backed contour (`formula_recognition_attempted=36`, `recognized=27`, `review_required=9`). После установки недостающего `ftfy` и повторного PaddleOCR rerun с cached model unit-level outcome на `421/пр` оказался идентичным для `tesseract` и `paddleocr`, то есть текущий gain здесь идёт из shared local WMF asset hints, а не из backend-specific OCR delta. Полный pipeline rerun на `SP_14.pdf` подтвердил тот же вывод уже на большом `pdf_text` stress-case: оба local backend-а дали одинаковый machine-readable package (`38,858` units, `226` table, `5` figure, `2` unresolved formula units), а после нормализации `processing.formula_recognition.local_backend` и `source.original_path` `document.v1.json` и `search_text.txt` совпали полностью; этот PDF полезен как conversion-stress/control case, но не как backend-specific OCR differentiator, потому что formulas здесь остаются `formula_text` без `asset_ref`.
...
151. Повторный запуск `/execute-production-roadmap-autonomous.prompt.md` на `main` подтвердил S10.1 blocker на текущем checkout: `unittest discover` прошёл `194` tests (`4` skipped), `scripts/validate_harness_assets.py` вернул `features=51`, `validated=51`, `active=0`, `telemetry_entries=98`; formula benchmark report `20260526T054732Z` всё ещё ниже GA floor (`gate.calc_expr_coverage=0.6776`, `gate.native_coverage=0.1858`), а telemetry span составляет только 10 календарных дней (`2026-05-22` .. `2026-05-31`).
...
Текущий фокус: S9.2 остаётся активным как hosted nightly burn-in contour, S9.3 уже реализован локально через tag-driven release workflow, PR #3 и PR #5 уже auto-merged в `main`. GitHub proof теперь есть в форме failed dispatch runs `26707002316`/`26707185880`/`26707569316`, issue #4 и зелёного PR CI `26707476363`; full formula monitor repair доказан на `main`, а текущий nightly success path требует merge ветки `agent/s9-2-nightly-table-source-guard`, потому что table anchor manifest указывает на локальные external sample inputs, недоступные hosted runner. Во время этого wait-state уже локально закрыта вся Wave 3: formula-recognition dispatch идёт через `FormulaProvider`, OCR scan path идёт через `OcrBackend`, root catalog emission идёт через `CatalogWriter`, а artifact serialization boundaries проходят через общий secret-redaction слой.
...
Новый provider-learning по S3.1: pluggable protocol начинает окупаться только когда fallback chain перестаёт быть ручной `if`-лесенкой в orchestration-коде. Явный `FormulaProvider` contract плюс `NullProvider` даёт тестируемый no-network path и убирает special-case knowledge о провайдере из postprocess loop.
