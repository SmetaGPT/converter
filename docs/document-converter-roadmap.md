...
Создать Windows-приложение, которое пользователь запускает как обычную программу, выбирает входную папку с DOCX, PDF и XLSX, выбирает выходную папку и получает переносимый пакет машиночитаемых документов.
...
- PDF-сканы;
- XLSX-книги с листами, ячейками, значениями и Excel-формулами;
...
- для XLSX сохраняется worksheet/cell provenance и structured `cell` payload с адресом, значением, Excel-формулой и числовым форматом;
...
- route detector: DOCX, PDF-text, PDF-scan, XLSX;
- DOCX converter: нативный разбор DOCX;
- XLSX converter: нативный разбор workbook/sheets/cells/formulas через openpyxl;
...
2. выбрать папку с DOCX/PDF/XLSX;
...
8. построить chunks по structural units без повторного чтения исходных DOCX/PDF/XLSX.
...
7. Post-v0.3.0: native XLSX route для workbook/sheet/cell/formula packages.
