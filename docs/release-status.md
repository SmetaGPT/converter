...
- formula-recognition/operator HTML follow-up: postprocess теперь покрывает и low-confidence `formula` units, packaged EXE ищет `.env.local` рядом с `sys.executable`, а human-readable HTML больше не отправляет в MathJax low-confidence heuristic formulas и показывает распознанные `formula_image` units как readable formula block со ссылкой на исходный asset.
