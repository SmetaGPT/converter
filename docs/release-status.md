...
Статус: production-ready within declared scope; W9 hosted nightly and tag-driven release proof completed; v1.0 GA remains blocked on S10.1 time-based and provider-assisted formula acceptance gates; local strict-config, provider-hardening and S11.4 coverage-expansion slices are closed, remaining blockers are S11.3 live provider evidence for the small formula corpus, missing opt-in provider credentials in the current workspace session, and the external time window
...
Следом закрыт и S9.2 nightly tranche: `.github/workflows/nightly-full-e2e.yml` запускает synthetic e2e, full formula benchmark monitor без thresholds, CI-safe formula gate, table-anchor source preflight с real table run при доступных external samples, EXE smoke и nightly portable package, а `scripts/create_nightly_failure_issue.py` создаёт failure issue с latest merged PR context и explicit/fallback `agent_id` trace. Hosted failure dispatches `26707002316`/`26707185880` доказали auto-issue path и открыли issue #4; guarded dispatch `26707811922` завершился `success` и загрузил `nightly-full-e2e-artifacts` (`7315270976`) со `skipped_missing_input` table-source preflight diagnostic.

Следом закрыт и S9.3 release-automation tranche: `.github/workflows/release.yml` публикует portable release по `v*` tag, `scripts/package-release.ps1` больше не держит hard-coded notes, а рендерит `release-notes.md` из git-tracked `CHANGELOG.md` через тестируемый `src/doc_converter/release_notes.py` / `scripts/render_release_notes.py`. Hosted run `26707894247` на tag `v0.3.0` completed `success` и опубликовал GitHub Release `https://github.com/SmetaGPT/converter/releases/tag/v0.3.0` с portable zip и checksum assets.

Параллельно с W9 hosted-evidence tranche локально закрыт и независимый S3.1: `src/doc_converter/formulas/providers.py` вводит `FormulaProvider` protocol и три реализации (`LocalTesseractProvider`, `OpenRouterProvider`, `NullProvider`), а `run_formula_recognition_postprocess` теперь использует provider chain вместо hard-coded backend/provider ветвления.
...
Текущий blocker для продолжения autonomous roadmap теперь S10.1, а не GitHub-hosted W9: nightly/tag evidence для `S9.2/S9.3` снят, source DOCX для `gate-metod-1-pr` больше не missing, локальный `1/пр` residue закрыт, а S11.2a Mathpix + LLM plumbing уже реализован. До v1.0 GA нельзя честно дойти в одной сессии, потому что exit criteria требуют 4 недели telemetry без unresolved regressions, 30 подряд зелёных portable EXE/package runs и provider-assisted formula GA evidence; следующий инженерный blocker — S11.2b operational provider gate (cache, budget/cost guardrails, explicit review semantics, live pilot) плюс S11.3 provider-assisted report. Текущий checked baseline даёт только 10 дней telemetry window и старый formula report `20260526T054732Z` с `gate.calc_expr_coverage=0.6776`, `gate.native_coverage=0.1858`.
...
- production roadmap S11.4 coverage expansion closeout: `samples/manifest.negative.jsonl` + `samples/expected/negative/` закрепляют no-table false-positive, broken-WMF review и protected-PDF review contours; `tests/test_property_based.py` добавляет Hypothesis coverage для formula calc normalizer и shared table row merger; Windows CI теперь пишет coverage report/xml без fail-under порога.

Следующий backlog:

- production roadmap S10.1: v1.0 gate assessment, formula benchmark threshold uplift и накопление time-based GA evidence.
...
- table benchmark follow-up на `sample_009`, `sample_018` и `sample_020`: next sprint должен не создавать baseline с нуля, а снижать `table_structure_warning` density на text-layer anchors и устранить OCR blocker на `sample_020`, чтобы scan route тоже вошёл в measured table loop; explicit synthetic negative/control contour уже зафиксирован S11.4.
...
Предыдущие critical-path risks по structured CLI/operator contract, structured runtime telemetry contract, input hardening baseline, missing leak gate, protected merge surface, nightly full e2e и hosted release automation закрыты в S6.1-S9.3; ближайший critical path теперь смещён на S10.1 v1.0 GA gates.
...
Nightly infrastructure заведена, видна на `main` и имеет hosted proof: PR #3 auto-merged, workflow registry показывает `nightly-full-e2e` и `release`, hosted dispatches `26707002316`/`26707185880` доказали failure issue path и открыли issue #4, PR #5 auto-merged после зелёного Windows CI run `26707476363`, fixed dispatch `26707569316` доказал UTF-8 safe/non-blocking full formula monitor, а PR #6 auto-merged после зелёного Windows CI run `26707710691`. Guarded nightly dispatch `26707811922` на `main` завершился `success`; table-anchor CI manifest по-прежнему ссылается на ignored local sample PDFs, но теперь это фиксируется как `skipped_missing_input` preflight artifact вместо blocking crash. Hosted runner по-прежнему не видит внешний `D:\ФСНБ\...` corpus, поэтому required regression gate остаётся на CI-safe subset.

Release automation path получил hosted proof: tag `v0.3.0` указывает на `17712cbd5fbc3bf543e6d40c5ea422d62a6a865a`, workflow run `26707894247` пересобрал package, прошёл EXE smoke, загрузил artifact и опубликовал GitHub Release `https://github.com/SmetaGPT/converter/releases/tag/v0.3.0` с `DocumentConverter-0.3.0-windows-portable.zip` и checksum. Remaining S9.3 risk закрыт.
...
- Hosted proof: release workflow run `26707894247` на tag `v0.3.0` completed success и опубликовал GitHub Release `https://github.com/SmetaGPT/converter/releases/tag/v0.3.0`.
- Assets: `DocumentConverter-0.3.0-windows-portable.zip` (`34,148,822` bytes) и `DocumentConverter-0.3.0-windows-portable.sha256.txt`.
