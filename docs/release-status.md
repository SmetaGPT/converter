...
Статус: production-ready within declared scope; W9 hosted nightly and tag-driven release proof completed; v1.0 GA remains blocked on S10.1 time-based and threshold acceptance gates
...
Текущий blocker для продолжения autonomous roadmap теперь S10.1, а не GitHub-hosted W9: nightly/tag evidence для `S9.2/S9.3` снят, source DOCX для `gate-metod-1-pr` больше не missing, локальный `1/пр` residue закрыт. До v1.0 GA нельзя честно дойти в одной сессии, потому что exit criteria требуют 4 недели telemetry без unresolved regressions, 30 подряд зелёных portable EXE/package runs и formula benchmark thresholds `>=80%` calc / `>=70%` native; текущий checked state даёт только 10 дней telemetry window и formula report `20260526T054732Z` с `gate.calc_expr_coverage=0.6776`, `gate.native_coverage=0.1858`.
