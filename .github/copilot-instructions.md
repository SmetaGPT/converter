...
4. Extended path (release / resume / подтверждённо кросс-модульная задача): сначала `docs/state-snapshot.md` (~1k токенов), затем grep-точечно по нужным state-докам.

## Гигиена документов

- `docs/current-status.md` и `docs/release-status.md` держать компактными (дайджест + ссылки). Разросшуюся историю уводить в `docs/archive/`, не раздувать сами файлы.
